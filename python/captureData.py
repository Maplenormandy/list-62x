...
if cap0.isOpened() and cap1.isOpened():
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            t = len(data)
            shutterRange = np.linspace(1.0, 531.0, 50)
            cap0.set(15, 1.0)
            cap0.set(14, 16.0)
            cap1.set(15, 1.0)
            cap1.set(14, 16.0)
            for k in range(len(shutterRange)):
                data.loc[t+k, 'Shutter 0'] = shutterRange[k]
                data.loc[t+k, 'Gain 0'] = 16.0
                data.loc[t+k, 'Shutter 1'] = shutterRange[k]
